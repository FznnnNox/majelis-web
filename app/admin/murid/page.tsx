...
// import { Murid, HafalanItem, initialDataMurid } from "./types";
...
//   const [muridList, setMuridList] = useState<Murid[]>(initialDataMurid);
...
//   const handleOpenHafalanModal = (murid: Murid) => {
//     setSelectedMuridHafalan(murid);
//     setIsHafalanModalOpen(true);
//   };

//   const handleSubmitMurid = (formData: Omit<Murid, "id" | "riwayatHafalan">) => {
//     if (editingMurid) {
//       setMuridList((prev) =>
//         prev.map((item) =>
//           item.id === editingMurid.id ? { ...item, ...formData } : item
//         )
//       );
//     } else {
...
//   // 🛠️ FIX LOGIC: CEK & UPDATE JIKA SURAH SUDAH ADA (MENGHINDARI DUPLIKASI)
//   const handleSaveHafalan = (muridId: number, addedItem: HafalanItem) => {
//     const updateMuridData = (m: Murid): Murid => {
//       // Cek apakah surah ini sudah ada sebelumnya di riwayat
//       const existingIndex = m.riwayatHafalan.findIndex(
//         (h) => h.surah.toLowerCase() === addedItem.surah.toLowerCase()
//       );

//       let updatedRiwayat: HafalanItem[];

//       if (existingIndex !== -1) {
//         // Jika sudah ada, update datanya dan taruh di urutan teratas
//         const filtered = m.riwayatHafalan.filter((_, idx) => idx !== existingIndex);
//         updatedRiwayat = [{ ...addedItem, id: m.riwayatHafalan[existingIndex].id }, ...filtered];
//       } else {
//         // Jika belum ada, tambahkan sebagai entri baru
//         updatedRiwayat = [addedItem, ...m.riwayatHafalan];
//       }

//       return {
//         ...m,
//         hafalanTerakhir: { surah: addedItem.surah, ayat: addedItem.ayat },
//         riwayatHafalan: updatedRiwayat,
//       };
//     };

//     setMuridList((prev) => prev.map((m) => (m.id === muridId ? updateMuridData(m) : m)));

//     setSelectedMuridHafalan((prev) => (prev && prev.id === muridId ? updateMuridData(prev) : prev));
//   };

//   const handleDelete = (id: number, nama: string) => {
//     if (confirm(`Hapus data pengajian "${nama}"?`)) {
...
//                 setIsModalOpen(true);
//               }}
//               onDeleteMurid={(id, nama) => {
//                 if (confirm(`Hapus data pengajian "${nama}"?`)) {
//                   setMuridList((prev) => prev.filter((i) => i.id !== id));
//                 }
//               }}
//             />
//           ))}
//         </div>
//       ) : (
//         <div className="bg-white/80 rounded-3xl p-8 sm:p-12 text-center border border-gray-100">
