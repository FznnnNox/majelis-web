...
//   { label: "Program Majelis", href: "#program" },
...
export default function Navbar() {
  ...
  useEffect(() => {
    // 1. Deteksi scroll background navbar
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 20);
    };

    // 2. Fungsi pembaca URL Hash paling akurat
    const syncHash = () => {
    ...
    // 3. Observer otomatis: Jika user scroll ke section #program di homepage
    let observer: IntersectionObserver | null = null;
    if (pathname === "/") {
      const targetSection = document.getElementById("program");
      if (targetSection) {
        observer = new IntersectionObserver(
          (entries) => {
            entries.forEach((entry) => {
              if (entry.isIntersecting) {
                setCurrentHash("#program");
  ...
  // Evaluasi menu mana yang wajib diberi underline oranye
  const isLinkActive = (href: string) => {
    if (href === "/#program") {
      return pathname === "/" && currentHash === "#program";
    }
    if (href === "/") {
      return pathname === "/" && currentHash !== "#program";
    }
